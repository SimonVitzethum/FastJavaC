...
/// Fixed header words before the instance fields (matches `runtime.c` `JObjHeader`
/// and the struct type emitted below — a **2-word** header):
///   Word 0: refcount (i64), packed count+flags; <0 = immortal. The cycle
///           collector's color/buffered bits live in these packed flags, NOT in
///           a separate word.
///   Word 1: vtable (ptr)
/// Instance fields therefore begin at LLVM struct GEP index 2 (== `HEADER_SLOTS`).
...
pub fn emit_debug(program: &Program, debug: Option<(&str, &str)>) -> String {
    ...
    writeln!(w, "@jrt_array_int_vt = external global ptr").unwrap();
    writeln!(w, "@jrt_array_ref_vt = external global ptr").unwrap();
    ...
    // For every class we emit an `FjcClass` descriptor (a superset of `@td.*`),
    // reachable at runtime by name through the external `@fjc_classes` table.
    // `runtime.c` reads these via `jrt_class_by_name`/`jrt_method`. The layout is
    // frozen under FJC_ABI_VERSION (PHASE-0-PLAN.md §2/§3). Emitted additively —
    // existing codegen (@vt/@td/@sf) is untouched. Offsets/sizes are computed by
    // LLVM itself via `ptrtoint(getelementptr …)` so they stay alignment-correct.
    {
        // Intern meta strings (field/method names + descriptors) into private,
        // NUL-terminated constants, deduped. Takes its buffers by &mut so repeated
        // sequential calls don't fight the borrow checker.
        let mut str_defs = String::new();
        let mut str_map: BTreeMap<String, String> = BTreeMap::new();
        let mut str_n = 0usize;
        let mut meta_str = |s: &str, defs: &mut String, map: &mut BTreeMap<String, String>, n: &mut usize| -> String {
            if let Some(sym) = map.get(s) {
                return sym.clone();
            }
            let sym = format!("@fjcs.{}", *n);
            *n += 1;
            let bytes = s.as_bytes();
            writeln!(defs, "{sym} = private unnamed_addr constant [{} x i8] c\"{}\\00\"", bytes.len() + 1, escape_ll(bytes)).unwrap();
            map.insert(s.to_string(), sym.clone());
            sym
        };
        // Reconstruct a JVM field descriptor from the IR type. Sub-int primitives
        // (byte/char/short/boolean) all lower to Ty::I32 in the IR, so they are
        // reported as "I"; primitive arrays lost their element type (ref_target
        // is None) and report "Ljava/lang/Object;". Exact descriptors would need
        // FieldInfo to carry the original descriptor (a later refinement).
        let field_desc = |f: &FieldInfo| -> String {
            match f.ty {
                Ty::I32 => "I".to_string(),
                Ty::I64 => "J".to_string(),
                Ty::F32 => "F".to_string(),
                Ty::F64 => "D".to_string(),
                Ty::Ref => match &f.ref_target {
                    Some(t) => format!("L{t};"),
        ...
        let mut class_syms: Vec<String> = Vec::new();
        let mut body = String::new();
        for c in &program.classes {
            let cs = sanitize(&c.name);
            let sn = ctx.struct_name(&c.name);

            // fields[]: declared instance fields (they sit at the tail of the
            // superclass-first flattened layout).
            let flat = ctx.flatten_fields(&c.name);
            let own_base = flat.len().saturating_sub(c.fields.len());
            let mut field_entries: Vec<String> = Vec::new();
            for (j, f) in c.fields.iter().enumerate() {
                let slot = HEADER_SLOTS + own_base + j;
                let name_sym = meta_str(&f.name, &mut str_defs, &mut str_map, &mut str_n);
                let desc_sym = meta_str(&field_desc(f), &mut str_defs, &mut str_map, &mut str_n);
                let off = format!("i32 trunc (i64 ptrtoint (ptr getelementptr ({sn}, ptr null, i32 0, i32 {slot}) to i64) to i32)");
                let is_ref = (f.ty == Ty::Ref) as u32;
                // Each struct element in an array-of-structs constant must be
                // prefixed with its type (LLVM aggregate-constant syntax).
                field_entries.push(format!("{{ ptr, ptr, i32, i32 }} {{ ptr {name_sym}, ptr {desc_sym}, {off}, i32 {is_ref} }}"));
            }
            let (fields_ptr, n_fields) = if field_entries.is_empty() {
                ("null".to_string(), 0)
            } else {
                writeln!(body, "@fjc.fields.{cs} = private unnamed_addr constant [{} x {{ ptr, ptr, i32, i32 }}] [{}]", field_entries.len(), field_entries.join(", ")).unwrap();
            ...
            for m in &c.methods {
                let name_sym = meta_str(&m.name, &mut str_defs, &mut str_map, &mut str_n);
                let desc_sym = meta_str(&m.desc, &mut str_defs, &mut str_map, &mut str_n);
                let code = if m.has_body && defined.contains(m.mangled.as_str()) {
                    format!("ptr @{}", m.mangled)
                } else {
                    "ptr null".to_string()
                };
                let vti: i32 = if m.is_virtual() {
                    ctx.vtable_index(&c.name, &m.name, &m.desc).map(|i| i as i32).unwrap_or(-1)
                } else {
                    -1
                };
                let mut mflags = 0u32;
                ...
                if !m.has_body {
                    mflags |= 2; // abstract or native (no Code attribute)
                }
                method_entries.push(format!("{{ ptr, ptr, ptr, i32, i32 }} {{ ptr {name_sym}, ptr {desc_sym}, {code}, i32 {vti}, i32 {mflags} }}"));
            }
            let (methods_ptr, n_methods) = if method_entries.is_empty() {
                ("null".to_string(), 0)
            } else {
                writeln!(body, "@fjc.methods.{cs} = private unnamed_addr constant [{} x {{ ptr, ptr, ptr, i32, i32 }}] [{}]", method_entries.len(), method_entries.join(", ")).unwrap();
                (format!("@fjc.methods.{cs}"), method_entries.len())
            };

            // ref_offsets[]: byte offsets of all ref fields (incl. inherited) — lets
            // the collector trace instances of dynamically loaded classes (Phase 2+).
            let ref_slots = ctx.ref_field_slots(&c.name);
            let (refoff_ptr, n_ref) = if ref_slots.is_empty() {
                ("null".to_string(), 0)
            } else {
                let entries: Vec<String> = ref_slots
                    .iter()
                    .map(|slot| format!("i32 trunc (i64 ptrtoint (ptr getelementptr ({sn}, ptr null, i32 0, i32 {slot}) to i64) to i32)"))
                    .collect();
                writeln!(body, "@fjc.refoff.{cs} = private unnamed_addr constant [{} x i32] [{}]", entries.len(), entries.join(", ")).unwrap();
                (format!("@fjc.refoff.{cs}"), ref_slots.len())
            };

            // ifaces[]: directly declared, modeled interfaces (as FjcClass pointers).
            let direct: Vec<&String> = c.interfaces.iter().filter(|i| program.class(i).is_some()).collect();
            let (ifaces_ptr, n_if) = if direct.is_empty() {
                ("null".to_string(), 0)
            } else {
                let entries: Vec<String> = direct.iter().map(|i| format!("ptr @fjc.{}", sanitize(i))).collect();
                writeln!(body, "@fjc.ifaces.{cs} = private unnamed_addr constant [{} x ptr] [{}]", entries.len(), entries.join(", ")).unwrap();
                (format!("@fjc.ifaces.{cs}"), direct.len())
            };

            let super_ptr = match &c.super_name {
                Some(s) if program.class(s).is_some() => format!("@fjc.{}", sanitize(s)),
                _ => "null".to_string(),
            };
            let (vt_ptr, vt_len) = if instantiated.contains(c.name.as_str()) {
                let len = VTABLE_METHOD_OFFSET + ctx.iface_slots.len() + ctx.vtable_slots(&c.name).len();
                (format!("@vt.{cs}"), len)
            } else {
                ("null".to_string(), 0)
            };
            ...
            let size = format!("i32 trunc (i64 ptrtoint (ptr getelementptr ({sn}, ptr null, i32 1) to i64) to i32)");
            ...
            .unwrap();
            class_syms.push(format!("@fjc.{cs}"));
        }

        // Emit the interned strings, the per-class descriptors, then the external
        // registry table the runtime looks up (external linkage → visible to C;
        // dropped by --gc-sections in pure-AOT builds that never introspect).
        write!(w, "{str_defs}").unwrap();
        write!(w, "{body}").unwrap();
        let n = class_syms.len();
        if n == 0 {
            writeln!(w, "@fjc_classes = constant [0 x ptr] zeroinitializer").unwrap();
        } else {
            let entries: Vec<String> = class_syms.iter().map(|s| format!("ptr {s}")).collect();
            writeln!(w, "@fjc_classes = constant [{n} x ptr] [{}]", entries.join(", ")).unwrap();
        }
        writeln!(w, "@fjc_classes_len = constant i32 {n}").unwrap();
        writeln!(w).unwrap();
    }
    ...
    if defined.contains("java_main") {
        ...
        // Open-world: publish the array vtables so JITted code can allocate arrays.
        if program.dynamic {
            writeln!(w, "  store ptr @vt.array.int, ptr @jrt_array_int_vt").unwrap();
            writeln!(w, "  store ptr @vt.array.ref, ptr @jrt_array_ref_vt").unwrap();
        }
