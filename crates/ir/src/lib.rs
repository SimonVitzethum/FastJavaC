...
/// Version of the frozen runtime ABI (object header layout, vtable slot ordering,
/// and the `FjcClass`/`FjcField`/`FjcMethod` descriptor layouts — see
...
pub struct Program {
    ...
    /// The auto motion-vector variant of the `@vertex` shader: the SAME body evaluated
    /// twice (current vs previous-frame uniform) writing `ndc_cur − ndc_prev` as the
    /// varying — the temporal input FSR2/DLSS need, generated by the compiler. `None`
    /// unless a program uses `vk_motion(...)`. See `compile_vertex_mv` in shader.rs.
    pub mv_vert_spvasm: Option<String>,
