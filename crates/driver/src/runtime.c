...
static int JIT_FRAME_BYTES = 0; /* current method's locals-frame size (8*max_locals) */
static void je_ret(void) {
    if (JIT_FRAME_BYTES > 0) { je1(0x48); je1(0x81); je1(0xC4); je_i32(JIT_FRAME_BYTES); } /* add rsp, frame */
    je1(0x5D); je1(0x5B); je1(0xC3);                                                        /* pop rbp; pop rbx; ret */
}
...
/* Read the 4-byte value of a CONSTANT_Integer/Float at cp index `idx` (the ldc/ldc_w
 * targets we can materialize inline). Returns 1 + raw bits; 0 for out-of-range or for
 * String/Class/other ldc constants (which need object creation — unsupported here). */
static int cf_cp_narrow(const uint8_t *buf, long flen, int idx, int32_t *out) {
    if (!buf || flen < 10 || idx <= 0) return 0;
    int cpc = cf_be16(buf + 8);
    long o = 10;
    for (int i = 1; i < cpc && o + 1 <= flen; i++) {
        uint8_t tag = buf[o];
        if (i == idx) {
            if ((tag == 3 || tag == 4) && o + 5 <= flen) { /* Integer / Float */
                *out = (int32_t)cf_be32(buf + o + 1);
                return 1;
            }
            return 0; /* String/Class/MethodHandle ldc: unsupported inline */
        }
        switch (tag) {
            case 1: o += 3 + cf_be16(buf + o + 1); break;
            case 7: case 8: case 16: case 19: case 20: o += 3; break;
            case 15: o += 4; break;
...
/* Build the constant-pool resolution tables (Class/Field/Method/NameAndType index maps)
 * for a class file into `out`, so getfield/putfield/new/invoke/instanceof can resolve
 * against the FjcClass registry. Allocates the index arrays (pointing INTO `f`, which must
 * outlive the CpInfo); release with cf_free_cp(). Returns 0 on success, negative on a
 * malformed/unsupported constant pool (nothing left allocated on failure). Kept separate
 * from cf_define_class's inline parse so the JIT-run harness gains the same resolution. */
static int cf_parse_cp(const uint8_t *f, long flen, CpInfo *out) {
    ...
    uint16_t *cls_ni = (uint16_t *)calloc(cpc, sizeof(uint16_t));
    uint16_t *ref_class = (uint16_t *)calloc(cpc, sizeof(uint16_t));
    uint16_t *ref_nat = (uint16_t *)calloc(cpc, sizeof(uint16_t));
    uint16_t *nat_name = (uint16_t *)calloc(cpc, sizeof(uint16_t));
    uint16_t *nat_desc = (uint16_t *)calloc(cpc, sizeof(uint16_t));
#define CF_CP_FREE_LOCAL() do { free(u); free(ul); free(cls_ni); free(ref_class); free(ref_nat); free(nat_name); free(nat_desc); } while (0)
    if (!u || !ul || !cls_ni || !ref_class || !ref_nat || !nat_name || !nat_desc) { CF_CP_FREE_LOCAL(); return -2; }
    ...
    for (int i = 1; i < cpc; i++) {
        ...
        switch (tag) {
            ...
            default: CF_CP_FREE_LOCAL(); return -3;
        }
    }
#undef CF_CP_FREE_LOCAL
    out->u = u; out->ul = ul; out->cpc = cpc; out->cls_ni = cls_ni;
    out->ref_class = ref_class; out->ref_nat = ref_nat;
    out->nat_name = nat_name; out->nat_desc = nat_desc;
    out->cf_buf = f; out->cf_len = flen;
    return 0;
}
static void cf_free_cp(CpInfo *cp) {
    free((void *)cp->u); free((void *)cp->ul); free((void *)cp->cls_ni);
    free((void *)cp->ref_class); free((void *)cp->ref_nat);
    free((void *)cp->nat_name); free((void *)cp->nat_desc);
}
...
static void jvm_delta(uint8_t op, const uint8_t *bc, int pc, const CpInfo *cp, int *pops, int *pushes, int *owned) {
    *pops = 0; *pushes = 0; *owned = 0;
    switch (op) {
        ...
        case 0x15 ... 0x19: case 0x1a ... 0x2d:
        case 0x59: *pushes = 1; break;                                   /* consts/loads/ldc/ldc2_w/dup */
        ...
        case 0x60 ... 0x73:                                             /* all i/l/f/d add/sub/mul/div/rem */
        case 0x78 ... 0x83: case 0x94: case 0x95 ... 0x98:
            *pops = 2; *pushes = 1; break;                               /* binary arith / shift / bitwise / cmp */
        case 0x74: case 0x75: case 0x85 ... 0x93: case 0xb4: case 0xc0: case 0xc1:
            *pops = 1; *pushes = 1; break;                               /* unary / conv / getfield / checkcast / instanceof */
...
/* Does the descriptor have any float/double argument? (Those go in xmm under the native
 * ABI, which the integer-register marshaller below does not handle.) */
static int desc_has_fp_args(const char *d) {
    const char *p = d; if (*p == '(') p++;
    while (*p && *p != ')') {
        if (*p == 'L') { while (*p && *p != ';') p++; if (*p) p++; }
        else if (*p == '[') { p++; }
        else { if (*p == 'F' || *p == 'D') return 1; p++; }
    }
    return 0;
}
...
/* Checked arithmetic + comparison leaves (defined later) the JIT calls so that
 * div-by-zero / INT_MIN÷-1 and NaN ordering match the AOT path exactly — one set
 * of semantics for both executors, no opcode-specific logic duplicated here. */
int32_t jrt_idiv(int32_t, int32_t);
int32_t jrt_irem(int32_t, int32_t);
int64_t jrt_ldiv(int64_t, int64_t);
int64_t jrt_lrem(int64_t, int64_t);
int32_t jrt_fcmpl(float, float);
int32_t jrt_fcmpg(float, float);
int32_t jrt_dcmpl(double, double);
int32_t jrt_dcmpg(double, double);
int32_t jrt_instanceof(void *, void *);
...
static void emit_native_marshal(int K) {
    static const char *movs[6] = {
        "\x48\x8B\xBC\x24", "\x48\x8B\xB4\x24", "\x48\x8B\x94\x24", /* rdi rsi rdx */
...
/* Native entry: spill the incoming argument registers into the locals frame at [RBX+8*slot]
 * (int/ref/long from RDI..R9, float/double from XMM0..7). Slot numbering follows the JVM
 * (long/double occupy two slots). `this` (instance methods) is the first integer arg. */
static void emit_entry_spill(const char *desc, int is_static) {
    static const char *isp[6] = { "\x48\x89\xBB", "\x48\x89\xB3", "\x48\x89\x93",   /* mov [rbx+d], rdi/rsi/rdx */
                                  "\x48\x89\x8B", "\x4C\x89\x83", "\x4C\x89\x8B" };  /* rcx/r8/r9 */
    static const unsigned char fmodrm[8] = { 0x83, 0x8B, 0x93, 0x9B, 0xA3, 0xAB, 0xB3, 0xBB }; /* [rbx+d], xmmN */
    int ireg = 0, freg = 0, slot = 0;
    if (!is_static) { jeN(isp[ireg++], 3); je_i32(8 * slot); slot++; } /* this -> slot 0 */
    const char *p = desc; if (*p == '(') p++;
    while (*p && *p != ')' && ireg < 6 && freg < 8) {
        char t = *p;
        if (t == '[') { while (*p == '[') p++; t = *p; if (t == 'L') { while (*p && *p != ';') p++; } if (*p) p++; t = 'L'; }
        else if (t == 'L') { while (*p && *p != ';') p++; if (*p) p++; t = 'L'; }
        else p++;
        if (t == 'F') { jeN("\xF3\x0F\x11", 3); je1(fmodrm[freg++]); je_i32(8 * slot); slot += 1; }
        else if (t == 'D') { jeN("\xF2\x0F\x11", 3); je1(fmodrm[freg++]); je_i32(8 * slot); slot += 2; }
        else if (t == 'J') { jeN(isp[ireg++], 3); je_i32(8 * slot); slot += 2; }
        else { jeN(isp[ireg++], 3); je_i32(8 * slot); slot += 1; }
    }
}
...
/* Native-call a runtime helper whose signature is given by a JVM descriptor: marshals
 * K operand-stack args (int/ref -> RDI.., float/double -> XMM..) and pushes the typed
 * result. Used for the fcmp/dcmp leaves (float/double in, int out). */
static void emit_call_jrt_desc(const void *fn, const char *desc, int K) {
    emit_native_marshal_desc(desc, K, 0);
    jeN("\x48\x89\xE5", 3);                        /* mov rbp, rsp */
    jeN("\x48\x83\xE4\xF0", 4);                    /* and rsp, -16 */
    je1(0x48); je1(0xB8); je_i64((int64_t)(uintptr_t)fn); /* mov rax, fn */
    jeN("\xFF\xD0", 2);                            /* call rax */
...
static jit_fn jit_compile_bc(const uint8_t *bc, int n, const uint8_t *exc, int nexc, const CpInfo *cp,
    ...
    /* Native-ABI prologue: save rbx+rbp (the pair keeps 16-byte call alignment), allocate a
     * locals frame on the stack, point RBX at it (survives C calls), and spill the incoming
     * native argument registers into the frame. So JITted methods look like AOT methods to
     * any caller (AOT or JIT) — the reverse-direction ABI. */
    int nloc = max_locals > 0 ? max_locals : 64;
    if (nloc > 4096) return NULL;
    JIT_FRAME_BYTES = 8 * nloc;
    je1(0x53); je1(0x55);                                  /* push rbx; push rbp */
    je1(0x48); je1(0x81); je1(0xEC); je_i32(JIT_FRAME_BYTES); /* sub rsp, frame */
    jeN("\x48\x89\xE3", 3);                                /* mov rbx, rsp (locals base) */
    emit_entry_spill(selfdesc ? selfdesc : "()V", self_static);
    ...
    while (pc < n) {
        ...
        switch (op) {
            ...
            /* idiv/irem/ldiv/lrem: call the checked leaves (div-by-zero throw + MIN/-1),
             * so semantics match AOT and no CPU #DE can crash JITted code. args a,b at
             * [rsp+8],[rsp] -> RDI,RSI; result in rax. */
            case 0x6c: emit_call_jrt((const void *)&jrt_idiv, 2, 1); pc++; break; /* idiv */
            case 0x70: emit_call_jrt((const void *)&jrt_irem, 2, 1); pc++; break; /* irem */
            case 0x6d: emit_call_jrt((const void *)&jrt_ldiv, 2, 1); pc++; break; /* ldiv */
            case 0x71: emit_call_jrt((const void *)&jrt_lrem, 2, 1); pc++; break; /* lrem */
            ...
            /* fcmpl/fcmpg/dcmpl/dcmpg: the l/g variants differ only on NaN (JVMS 6.5);
             * the leaves encode that, called with float/double args via xmm. */
            case 0x95: emit_call_jrt_desc((const void *)&jrt_fcmpl, "(FF)I", 2); pc++; break; /* fcmpl */
            case 0x96: emit_call_jrt_desc((const void *)&jrt_fcmpg, "(FF)I", 2); pc++; break; /* fcmpg */
            case 0x97: emit_call_jrt_desc((const void *)&jrt_dcmpl, "(DD)I", 2); pc++; break; /* dcmpl */
            case 0x98: emit_call_jrt_desc((const void *)&jrt_dcmpg, "(DD)I", 2); pc++; break; /* dcmpg */
            ...
            case 0x12: case 0x13: { /* ldc / ldc_w: int/float constant (String/Class refs unsupported) */
                int cidx = (op == 0x12) ? bc[pc + 1] : ((bc[pc + 1] << 8) | bc[pc + 2]);
                int32_t v;
                if (!cp || !cf_cp_narrow(cp->cf_buf, cp->cf_len, cidx, &v)) return NULL;
                je_push_imm(v); pc += (op == 0x12) ? 2 : 3; break;
            }
            ...
            case 0xc1: { /* instanceof: push (objref instanceof T) ? 1 : 0 */
                const FjcClass *tc = jit_resolve_class(cp, (bc[pc + 1] << 8) | bc[pc + 2]);
                if (!tc || !tc->vtable) return NULL;            /* need a materialized type descriptor */
                void *td = ((void *const *)tc->vtable)[2];      /* vtable[2] = TypeDesc* (cf. jrt_instanceof) */
                if (!td) return NULL;
                je1(0x5F);                                      /* pop rdi (objref) */
                je1(0x48); je1(0xBE); je_i64((int64_t)(uintptr_t)td); /* mov rsi, td */
                jeN("\x48\x89\xE5", 3);                          /* mov rbp, rsp */
                jeN("\x48\x83\xE4\xF0", 4);                      /* and rsp, -16 */
                je1(0x48); je1(0xB8); je_i64((int64_t)(uintptr_t)&jrt_instanceof); /* mov rax, fn */
                jeN("\xFF\xD0", 2);                              /* call rax */
                jeN("\x48\x89\xEC", 3);                          /* mov rsp, rbp */
                je1(0x50);                                       /* push rax (0/1) */
                pc += 3; break;
            }
            ...
            /* float/double arrays: the JIT keeps floats/doubles as raw bits in the
             * 64-bit operand slots, and float[]/double[] have 4/8-byte element stride,
             * so the int/long array leaves move exactly the right bytes (bit-identical
             * to a real f/daload). No xmm needed — pure memory move. */
            case 0x30: emit_call_jrt((const void *)&jrt_iaload, 2, 1); pc++; break;  /* faload  = iaload  (4B bits) */
            case 0x51: emit_call_jrt((const void *)&jrt_iastore, 3, 0); pc++; break; /* fastore = iastore (4B bits) */
            case 0x31: emit_call_jrt((const void *)&jrt_laload, 2, 1); pc++; break;  /* daload  = laload  (8B bits) */
            case 0x52: emit_call_jrt((const void *)&jrt_lastore, 3, 0); pc++; break; /* dastore = lastore (8B bits) */
            ...
            case 0xb7: case 0xb8: {
                ...
                /* Unified native call (JIT methods now have a native entry, so both AOT and
                 * JIT callees take native args). Code is loaded through &FjcMethod at runtime
                 * so forward/self references (m->code still NULL at compile time, filled at
                 * pass 2) resolve. Only a fully UNRESOLVABLE target (m==NULL, e.g. the
                 * unmodeled java.lang.Object.<init>) is a no-op / unsupported. */
                if (!m) {
                    /* Unmodeled void constructor (e.g. java.lang.Object.<init>): no-op — pop args. */
                    int vr0 = 0, ka0 = jit_arg_count(mdesc, &vr0);
                    if (ka0 < 0) return NULL;
                    int K0 = ka0 + (op == 0xb7 ? 1 : 0);
                    if (op == 0xb7 && vr0) { if (K0 > 0) { je1(0x48); je1(0x81); je1(0xC4); je_i32(8 * K0); } pc += 3; break; }
                    return NULL;
                }
                int voidret = 0, kargs = jit_arg_count(mdesc, &voidret);
                if (kargs < 0) return NULL;
                int K = kargs + (op == 0xb7 ? 1 : 0); /* invokespecial: + receiver */
                if (!marshal_feasible(mdesc, op == 0xb7)) return NULL;
...
static const uint8_t *cf_find_code(const uint8_t *f, long flen, const char *mname, const char *mdesc,
                                   int *out_len, const uint8_t **out_exc, int *out_nexc, int *out_maxlocals) {
    *out_exc = NULL; *out_nexc = 0; *out_maxlocals = 0;
...
int32_t jrt_jit_run(const void *path_j, const void *mname_j, const void *mdesc_j, int32_t arg) {
     ...
     * Build the full CpInfo so field/method/class resolution works exactly as in the
     * defineClass path — getfield/putfield/new/invoke/instanceof resolve against the
     * registry. Falls back to a classfile-only CpInfo (ldc2_w still resolves) if the CP
     * is malformed. */
    CpInfo cpi;
    int have_cp = (cf_parse_cp(buf, flen, &cpi) == 0);
    if (!have_cp) { memset(&cpi, 0, sizeof cpi); cpi.cf_buf = buf; cpi.cf_len = flen; }
    jit_fn fn = jit_compile_bc(code, codelen, jexc, njexc, &cpi, mdesc, 1, jmaxloc);
    if (have_cp) cf_free_cp(&cpi);
    free(buf); /* bytecode already copied into the JITted page; CP tables freed above */
    if (!fn) return -7;
    return ((int32_t (*)(int64_t))(void *)fn)((int64_t)arg);
}

/* Array header (matches the backend): { i64 refcount, ptr vtable, i64 length,
 * i64 elem_size, [data] }. Length at +16, byte data at +32 (== jrt_array_data). */
#define JARR_LEN(a) (*(const int64_t *)((const char *)(a) + 16))
#define JARR_DATA(a) ((const uint8_t *)(a) + 32)

/* File helpers so a host can pull a class file into a Java byte[] and then define it
 * from memory (the byte[] is allocated in Java; C only fills it). */
int32_t jrt_file_size(const void *path_j) {
    char path[4096];
    jstr_to_cstr(path_j, path, sizeof path);
    if (!path[0]) return -1;
    FILE *fp = fopen(path, "rb");
    ...
    long n = ftell(fp);
    fclose(fp);
    return (n < 0 || n > (1 << 30)) ? -3 : (int32_t)n;
}
int32_t jrt_read_into(const void *arr, const void *path_j) {
    if (!arr) return -1;
    char path[4096];
    jstr_to_cstr(path_j, path, sizeof path);
    FILE *fp = fopen(path, "rb");
    ...
    uint8_t *data = (uint8_t *)((char *)arr + 32);
    size_t got = fread(data, 1, (size_t)(cap > 0 ? cap : 0), fp);
    fclose(fp);
    return (int32_t)got;
}
 ...
 * runtime-generated bytecode becomes native machine code in-process. This is the
 * bridge to runtime bytecode generation. */
int32_t jrt_jit_raw(const void *bc_arr, int32_t arg) {
    if (!bc_arr) return -1;
    int64_t len = JARR_LEN(bc_arr);
    if (len <= 0 || len > (1 << 16)) return -2;
    /* Raw blob: a static (I)I method by convention — one int arg in RDI, 64 locals. */
    jit_fn fn = jit_compile_bc(JARR_DATA(bc_arr), (int)len, NULL, 0, NULL, "(I)I", 1, 64);
    if (!fn) return -3;
    return ((int32_t (*)(int64_t))(void *)fn)((int64_t)arg);
}

/* defineClass(byte[]) proper: an in-memory CLASS FILE (byte[]) — the exact input a
 * ClassLoader.defineClass receives — from which we locate a method and JIT it. Same
 * engine as jrt_jit_run, but the bytes come from memory rather than a file, so bytecode
 * a loader/transformer produced at runtime can be defined and run. */
int32_t jrt_define_and_run(const void *class_arr, const void *m_j, const void *d_j, int32_t arg) {
    if (!class_arr) return -1;
    int64_t len = JARR_LEN(class_arr);
    if (len <= 0 || len > (16 << 20)) return -2;
    char mname[256], mdesc[256];
    jstr_to_cstr(m_j, mname, sizeof mname);
    jstr_to_cstr(d_j, mdesc, sizeof mdesc);
    if (!mname[0] || !mdesc[0]) return -3;
    int codelen = 0, jmaxloc = 0;
    const uint8_t *jexc = NULL; int njexc = 0;
    const uint8_t *code = cf_find_code(JARR_DATA(class_arr), len, mname, mdesc, &codelen, &jexc, &njexc, &jmaxloc);
    if (!code || codelen <= 0) return -6;
    jit_fn fn = jit_compile_bc(code, codelen, jexc, njexc, NULL, mdesc, 1, jmaxloc);
    if (!fn) return -7;
    return (int32_t)((int64_t (*)(int64_t))(void *)fn)((int64_t)arg);
}
...
#define CF_FREE_CP() do { free(u); free(ul); free(cls_ni); free(ref_class); free(ref_nat); free(nat_name); free(nat_desc); } while (0)
    ...
    CpInfo cpinfo = { u, ul, cpc, cls_ni, ref_class, ref_nat, nat_name, nat_desc, f, flen };
    uint16_t this_class = cf_be16(p + 2);          /* after access(2) */
    uint16_t super_class = cf_be16(p + 4);
    p += 6;                                        /* access, this, super */
    p += 2 + 2 * cf_be16(p);                       /* interfaces */
    uint16_t fc = cf_be16(p); p += 2;              /* fields */
    for (int i = 0; i < fc; i++) { p += 6; uint16_t ac = cf_be16(p); p += 2; for (int a = 0; a < ac; a++) { uint32_t al = cf_be32(p + 2); p += 6 + al; } }
    ...
    int *mmaxloc = (int *)calloc(mc ? mc : 1, sizeof(int));
    FjcClass *c = (FjcClass *)calloc(1, sizeof(FjcClass));
    const FjcClass **arr = (const FjcClass **)calloc(1, sizeof(void *));
    if (!methods || !mcode || !mclen || !mexcp || !mexcn || !mmaxloc || !c || !arr || !name) {
        CF_FREE_CP(); free(methods); free(mcode); free(mclen); free(mexcp); free(mexcn); free(mmaxloc); free(c); free(arr); free(name);
        return -4;
    }
    ...
    for (int i = 0; i < mc; i++) {
        ...
        uint16_t ac = cf_be16(p); p += 2;
        const uint8_t *code = NULL; int codelen = 0, ml = 0;
        ...
        methods[nm].flags = ((maf & 0x0008) ? 1u : 0u) | 4u; /* bit0 ACC_STATIC, bit2 = JIT-defined */
        mcode[nm] = code; mclen[nm] = codelen; mexcp[nm] = mexc; mexcn[nm] = nmexc; mmaxloc[nm] = ml;
        nm++;
    }
    ...
    /* If the class extends a registered (AOT) class with a vtable, give it one now (copy the
     * super's, override slots filled after pass 2) — BEFORE JITting bodies, since `new` bakes
     * the vtable pointer. So JIT-defined polymorphic subclasses work: their overrides are
     * installed in the vtable and dispatched (now correctly, with the native ABI + JIT RC).
     * Constraint: no added instance fields (same layout / drop-trace as the super). */
    const FjcClass *cvt_super = NULL;
    if (super_class < cpc && cls_ni[super_class] && cls_ni[super_class] < cpc) {
        uint16_t sni = cls_ni[super_class];
        char sd[256]; int sl = ul[sni];
        if (sl > 0 && sl < (int)sizeof sd) {
            for (int i = 0; i < sl; i++) { char ch = (char)u[sni][i]; sd[i] = ch == '/' ? '.' : ch; }
            sd[sl] = '\0';
            cvt_super = jrt_class_by_name(sd);
        }
    }
    if (cvt_super && cvt_super->vtable && cvt_super->vtable_len > 0) {
        void **vt = (void **)malloc((size_t)cvt_super->vtable_len * sizeof(void *));
        if (vt) {
            memcpy(vt, cvt_super->vtable, (size_t)cvt_super->vtable_len * sizeof(void *));
            c->vtable = vt; c->vtable_len = cvt_super->vtable_len;
            c->instance_size = cvt_super->instance_size; c->super = cvt_super;
        }
    }
    ...
    for (int j = 0; j < nm; j++) {
        if (mcode[j] && mclen[j] > 0) {
            jit_fn fn = jit_compile_bc(mcode[j], mclen[j], mexcp[j], mexcn[j], &cpinfo,
                                       methods[j].desc, (methods[j].flags & 1) ? 1 : 0, mmaxloc[j]);
            if (fn) { methods[j].code = (void *)fn; jitted++; }
        }
    }

    /* Install this class's overrides into its vtable (code now exists), so virtual calls on
     * its instances dispatch to the overrides (the mod/Mixin pattern). */
    if (c->vtable && cvt_super) {
        void **vt = (void **)c->vtable;
        for (int j = 0; j < nm; j++) {
            if ((methods[j].flags & 1) || !methods[j].code) continue; /* static / not JITted */
            for (const FjcClass *s = cvt_super; s; s = s->super) {
                const FjcMethod *sm = jrt_method(s, methods[j].name, methods[j].desc);
                if (sm && sm->vtable_index >= 0 && (uint32_t)sm->vtable_index < c->vtable_len) {
                    vt[sm->vtable_index] = (void *)methods[j].code;
                    methods[j].vtable_index = sm->vtable_index;
                    break;
                }
            }
        }
    ...
    CF_FREE_CP();
    free(mcode); free(mclen); free(mexcp); free(mexcn); free(mmaxloc);
    return jitted;
}
...
int64_t jrt_call_static(const void *cls_j, const void *m_j, const void *d_j, int32_t arg) {
    ...
    int64_t r = ((int64_t (*)(int64_t))(void *)mm->code)((int64_t)arg);
...
int32_t jrt_call_static_obj1(const void *cls_j, const void *m_j, const void *d_j, const void *obj) {
    ...
    return ((int32_t (*)(const void *))(void *)mm->code)(obj);
}
...
void *jrt_call_static_ref(const void *cls_j, const void *m_j, const void *d_j, const void *obj) {
    ...
    void *res = ((void *(*)(const void *))(void *)mm->code)(obj);
...
double jrt_call_double(const void *cls_j, const void *m_j, const void *d_j, double arg) {
    ...
    return ((double (*)(double))(void *)mm->code)(arg);
}
...
float jrt_call_float(const void *cls_j, const void *m_j, const void *d_j, float arg) {
    ...
    return ((float (*)(float))(void *)mm->code)(arg);
}
...
int32_t jrt_jit_raw(const void *a, int32_t b) { (void)a; (void)b; return -100; }
int32_t jrt_define_and_run(const void *a, const void *b, const void *c, int32_t d) {
...
#include <ffi.h>
#include <stdarg.h>
static ffi_type *ffi_of(char t);   /* fwd (defined with the general FFI call below) */
...
/* --- Method callbacks: native code calls back into Java. GetMethodID → &FjcMethod;
 * Call<T>Method dispatches virtually (obj vtable[vtable_index]) or directly (static/
 * exact m->code) and libffi-calls the fastjavac native ABI (this in arg0). Varargs
 * are promoted per JNI (int/double); we narrow F back and pass real types to ffi. */
static const FjcMethod *jni_find_method(const FjcClass *c, const char *name, const char *sig) {
    while (c) {
        for (uint32_t i = 0; i < c->n_methods; i++)
            if (jrt_cstreq(c->methods[i].name, name) && jrt_cstreq(c->methods[i].desc, sig))
                return &c->methods[i];
        c = c->super;
    }
    return NULL;
...
static int64_t jni_call_va(void *recv, const FjcMethod *m, va_list ap, char ret) {
    ...
    while (*p && *p != ')' && n < 34) {
        ...
        switch (kind) {
            case 'J': st[n].j = va_arg(ap, int64_t); at[n] = &ffi_type_sint64; av[n] = &st[n].j; break;
            case 'F': st[n].f = (float)va_arg(ap, double); at[n] = &ffi_type_float; av[n] = &st[n].f; break;
            case 'D': st[n].d = va_arg(ap, double); at[n] = &ffi_type_double; av[n] = &st[n].d; break;
            case 'L': st[n].p = va_arg(ap, void *); at[n] = &ffi_type_pointer; av[n] = &st[n].p; break;
            default:  st[n].i = va_arg(ap, int); at[n] = &ffi_type_sint32; av[n] = &st[n].i; break;
        }
        n++; p++;
    }
